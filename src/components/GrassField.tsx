...
// Dynamic mobile checks and performance tuning
const isMobileDevice = typeof window !== 'undefined' && (
  /Mobi|Android|iPhone|iPad|Macintosh/i.test(navigator.userAgent) && 'ontouchend' in document ||
  /Mobi|Android|iPhone|iPad/i.test(navigator.userAgent) ||
  window.innerWidth < 768
);

// WebGPURenderer transparently runs on a WebGL2 backend when WebGPU is missing
// (Safari < 26, older browsers). `?forceWebGL` lets us exercise that path anywhere.
const FORCE_WEBGL = typeof window !== 'undefined' && new URLSearchParams(window.location.search).has('forceWebGL');
const USE_WEBGPU = typeof navigator !== 'undefined' && 'gpu' in navigator && !FORCE_WEBGL;

// Blade counts must stay perfect squares (grid layout). The WebGL2 backend runs the
// same TSL compute via transform feedback but with less headroom, so budget lower.
const BLADE_COUNT = isMobileDevice
  ? (USE_WEBGPU ? 14400 : 10000)   // 120² / 100²
  : (USE_WEBGPU ? 81225 : 36100);  // 285² / 190²
const GRID_SIZE = Math.sqrt(BLADE_COUNT);
const MAX_PIXEL_RATIO = USE_WEBGPU ? 2 : 1.5;
const FIELD_SIZE = isMobileDevice ? 32 : 60; // scale size downward to preserve density on mobile
...
export const GrassField: React.FC = () => {
  ...
  const [useFallback, setUseFallback] = useState(() => {
    ...
    if (USE_WEBGPU) return false;
    // No WebGPU: the renderer will run on its WebGL2 backend (works in Safari).
    // Only drop to the 2D canvas when even WebGL2 is unavailable.
  ...
  useEffect(() => {
    if (!useFallback || !canvasRef2D.current) return;
    ...
    // Beautiful flowing embers system
    ...
    const draw = () => {
      ...
      // Deep dark crimson ambient aura rising from the bottom center
      ...
      radialGlow.addColorStop(0, 'rgba(100, 0, 18, 0.22)');
      radialGlow.addColorStop(0.5, 'rgba(25, 0, 4, 0.08)');
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        const alpha = p.opacity * (0.65 + 0.35 * Math.sin(p.pulsePhase));
        ...
        pGlow.addColorStop(0, `rgba(250, 0, 63, ${alpha})`); // Cinematic brand red FA003F
        pGlow.addColorStop(0.3, `rgba(180, 0, 40, ${alpha * 0.55})`);
        pGlow.addColorStop(1, 'rgba(250, 0, 63, 0)');
  ...
  // --- Main Three.js WebGPU Grass Engine ---
  useEffect(() => {
    ...
    try {
      ...
      renderer = new THREE.WebGPURenderer({
        antialias: USE_WEBGPU, // DoF already softens edges; skip MSAA on the WebGL2 path
        forceWebGL: !USE_WEBGPU,
      ...
      const mouseStrength = uniform(12.0);
      const outerRadius = uniform(8.0);
      ...
      const computeInit = Fn(() => {
        ...
        const gridSize = float(GRID_SIZE);
        ...
        const maxR = float(FIELD_SIZE * 0.42).add(edgeNoise.sub(0.5).mul(10.0));
      ...
      const computeUpdate = Fn(() => {
        ...
        const lw = deltaTime.mul(2.0).saturate();
        ...
        const lm = select(targetMag.greaterThan(currentMag), deltaTime.mul(12.0), deltaTime.mul(0.8)).saturate();
      ...
      function createBladeGeometry() {
        const segs = isMobileDevice ? 4 : 5, W = 0.055, H = 1.0;
      ...
      const initRenderer = async () => {
        try {
          ...
          // Reused across frames — no allocations inside the render loop
          const targetPos = new THREE.Vector3();
          const currentLookTarget = new THREE.Vector3();

          // Dynamic resolution: measure real frame rate and trade pixels for
          // smoothness (and back) so heavy scenes stay responsive on weaker GPUs.
          let dprScale = 1;
          let frameAccum = 0;
          let frameCount = 0;
          let goodWindows = 0;
          const applyResolution = () => {
            renderer!.setPixelRatio(basePixelRatio * dprScale);
          ...
          const animate = () => {
            ...
            // Throttled background frames must not count against the average
            if (!document.hidden) {
              frameAccum += dt;
              frameCount++;
            }
            if (frameCount >= 60) {
              const avgFps = frameCount / frameAccum;
              if (avgFps < 45 && dprScale > 0.55) {
                dprScale = Math.max(0.55, dprScale - 0.15);
                goodWindows = 0;
                applyResolution();
              } else if (avgFps > 57 && dprScale < 1) {
                // Only scale back up after two consecutive smooth windows
                goodWindows++;
                if (goodWindows >= 2) {
            ...
            targetPos.set(cam.px, cam.py, cam.pz);
            currentLookTarget.set(cam.lx, cam.ly, cam.lz);
          ...
          resumeLoop = () => {
            // The browser throttles rAF while hidden; on return, discard the
            // hidden time and reset FPS sampling so resolution doesn't drop
            // based on throttled frames.
            clock.getDelta();
            frameAccum = 0;
            frameCount = 0;
            goodWindows = 0;
          ...
          console.warn("GPU compilation failed, falling back to 2D amber dust", innerError);
          setUseFallback(true);
        }
      };
      ...
      console.warn("WebGPURenderer fail-catch, enabling 2D fallback path", err);
      setUseFallback(true);
  ...
  if (useFallback) {
    return <canvas ref={canvasRef2D} className="absolute inset-0 h-full w-full object-cover" />;
  }
